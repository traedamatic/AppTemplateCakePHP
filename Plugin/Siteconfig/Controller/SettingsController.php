	...
	public function manager_add() {
		if($this->request->is('post')) {
			...
			if($this->Setting->save(array('SiteSettings' => $settings))) {
				$this->Session->setFlash(__('Setting saved'),'/flash/success');
			} else {				
				$this->Session->setFlash(__('Setting could not be saved saved'),'/flash/error');
			}
		...
		if(isset($settingKeys[0])) {			
			...
			if(isset($settingKeys[1])) {
				if(isset($settings[$settingKeys[0]][$settingKeys[1]])) {
					...
					$this->Session->setFlash(__('Setting not found'),'/flash/error');
				}
			} else {
			...
			if($this->Setting->save(array('SiteSettings' => $settings))) {
				...
				$this->Session->setFlash(__('Setting could not be deleted'),'/flash/error');
			}
						
		} else {
			$this->Session->setFlash(__('Setting Key not found'),'/flash/error');
		}
